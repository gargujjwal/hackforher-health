...
function PatientEditFormPage() {
  ...
  const updatePatientProfileMutationObject = updatePatientProfileMut(user.id);
  ...
  const {
    ...
    handleSubmit,
    setValue,
  } = useForm<PatientProfileDto>({ defaultValues: patientProfile });
  const {
  ...
  } = useArrayInput<string>({ initialValues: patientProfile?.phoneNumber });
  const {
    ...
    initialValues: patientProfile?.currentMedications,
  });
  const {
    fields: allergies,
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <Controller
              ...
              render={({ fieldState, field: { value, onChange } }) => (
                <Select
                  defaultSelectedKeys={[
                    patientProfile ? (patientProfile.isSmoker ? "1" : "0") : "",
                  ]}
...
export default PatientEditFormPage;
